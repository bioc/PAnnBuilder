HomoloGeneBuilder_DB <- function( 
    prefix, pkgPath, version, author
    ) {                        
  ## Build annotation data packages for homolog protein group.
  ## 
  ...
  srcUrls <- getSrcUrl("HomoloGene")
  built <- getSrcBuilt("HomoloGene")  
  ...
  # write data
  writeData_DB("HOMOLOGENE", srcUrls, db)  
  repList <- getRepList_DB(organism="", "HOMOLOGENE", srcUrls, built, pkgName)
